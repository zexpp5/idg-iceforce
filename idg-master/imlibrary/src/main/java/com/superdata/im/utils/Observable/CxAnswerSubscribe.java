 ...
 * @version v1.0.0
 */
public class CxAnswerSubscribe implements Observer{
    private CxUpdateUICallback callback;

    public CxAnswerSubscribe(CxUpdateUICallback callback){
    ...
    public void update(Observable observable, Object data) {
        if(data instanceof CxMessage){
            ...
            if(cxMessage.getType() == CxIMMessageType.ANSWER.getValue()){
                dealData();
                if(callback != null){
