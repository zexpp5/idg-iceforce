...
public class FileSelectionActivity extends Activity
{
    ...
    View ok, llLeft;
    // all, cancel, storage, New;
    TextView path;
    Boolean Switch = false;
    private ArrayList<File> selectFileList = new ArrayList<File>();
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        List<File> files = (List<File>) getIntent().getSerializableExtra("list");
        if (files != null)
        {
            selectFileList.addAll(files);
        }
        loadLists();
        ...
        directoryView.setOnItemClickListener(new OnItemClickListener()
        {
            public void onItemClick(AdapterView<?> parent, View view, int position, long id)
            {
                ...
                if (position - directoryList.size() >= 0)
                    if (directoryView.isItemChecked(position))
                    {
                        selectFileList.add(fileList.get(position - directoryList.size()));
                    } else
                    {
                        selectFileList.remove(fileList.get(position - directoryList.size()));
                    }
                try
                {
        ...
        // public void onClick(View v) {
        // if(!Switch){
        // for (int i = directoryList.size(); i < directoryView.getCount();
        // i++){
    ...
    public void back(View view)
    {
        try
        {
            File parentFile = mainPath.getParentFile();
            if (parentFile == null)
            {
                finish();
            } else
    ...
    public void ok()
    {
        ...
        result.putExtra(FILES_TO_UPLOAD, selectFileList);
    ...
    private void loadLists()
    {
        ...
        ArrayAdapter<String> directoryAdapter = new ArrayAdapter<String>(this, android.R.layout.simple_list_item_1,
                directoryNames);
        ...
        for (int i = 0; i < fileList.size(); i++)
        {
            ...
            if (selectFileList.contains(fileList.get(i)))
            {
                directoryView.setItemChecked(i + directoryList.size(), true);
            }
        }
        // }
    }
    ...
    public void iconload()
    {
        ...
        CustomListSingleOnly adapter1 = new CustomListSingleOnly(FileSelectionActivity.this, directoryNames.toArray
                (foldernames), mainPath.getPath());
