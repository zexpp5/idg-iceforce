...
public class SuperMainActivity extends BaseActivity implements View.OnClickListener
{
    ...
    protected void init()
    {
        JPushManager.getInstance(BaseApplication.getInstance()).setAlias(DisplayUtil.getUserInfo(SuperMainActivity.this, 5));
        ...
        ToolMainUtils.getInstance().setPSUnRead(SuperMainActivity.this);
    }
                ...
                if (!ownerBean.getUserId().equals(currentAccount))
                {
                    if (StringUtils.notEmpty(imGroup))
                    {
                        if (StringUtils.notEmpty(imGroup.getGroupName()))
                            MyToast.showToast(SuperMainActivity.this, "你已经被" + imGroup.getGroupName() + "移除");
                    } else
                    {
                        MyToast.showToast(SuperMainActivity.this, "你已经被移出群聊");
                    }
                }
            } catch (Exception e)
            {
...
//                    reFreshUnreadCount();
    ...
    {
        BackUtils.isBack(SuperMainActivity.this);
    }
}
