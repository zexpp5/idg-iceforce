...
import com.superdata.marketing.support.audio.SpeexEncoder;

public class AudioRecordHandler implements Runnable
{
    private int MAX_RECORDER = 60 * 10 * 10;
    ...
    public AudioRecordHandler(Handler uiHandler, String fileName)
    {
        super();
        this.fileName = fileName;
        ...
        try
        {
            ...
            try
            {
                ...
                while (this.isRecording)
                {
                    ...
                    SDLogUtil.debug("im_录音时间：" + recordTime);
                    if (recordTime >= Config.MAX_SOUND_RECORD_TIME)
                    {
                        if (audioCallback != null)
                        {
                            audioCallback.RecorderTooLong();
                            setLongRecordTime();
                        }
                        break;
                    }
    ...
    private void setLongRecordTime()
    {
        Message Msg = new Message();
        Msg.what = Constants.RECORD_AUDIO_TOO_LONG;
    ...
    private void setMaxVolume(short[] buffer, int readLen)
    {
        try
        {
            ...
            if (audioCallback != null)
            {
                audioCallback.getMaxAmplitude(max);
            }
            Message Msg = new Message();
            Msg.what = Constants.RECEIVE_MAX_VOLUME;
            Msg.obj = max;
            uiHandler.sendMessage(Msg);
        } catch (Exception e)
        {
    ...
    {
        void RecorderTooLong();

        void getMaxAmplitude(int volume);
    }
