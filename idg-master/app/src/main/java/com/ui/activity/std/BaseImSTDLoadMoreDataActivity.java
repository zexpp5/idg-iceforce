 ...
 *
 */
public abstract class BaseImSTDLoadMoreDataActivity extends BaseXListViewActivity {
    ...
    protected int type = 2;//2=上拉, !2=下拉
    protected long lastTime;
    protected long refreshTime;
    protected long moreTime;
...
//        });
    ...
    protected void getData(int status) {
        if (status == FIRST_LOAD) {
            currPage = 1;
        } else if (status == LOAD_MORE) {//LOAD_MORE
            // type = 2;
            type = 1;
...
//                SDToast.showShort("没有数据了");
        ...
        } else if (REFRESH == status) {//REFRESH
            //type = 1;
            type = 2;
            lastTime = refreshTime;
            if (currPage > 1) {
    ...
    protected abstract class BaseImRequestCallBack extends JsonCallback {
        ...
        public void onError(Request request, Exception e) throws Exception {
            int statusCode = request.hashCode();
            if (statusCode == 501) {
                mListView.setPullLoadEnable(false);
                ...
                onFailure(getCurrStatus());
            }
        }

        //        else if (pageNum <= currPage) { // 不能加载更多
//            setLoadMore(false);
//            mListView.setPullLoadEnable(false);
...
//        }
        ...
        public void onResponse(JSONObject response) throws Exception {
            ...
            if (getCurrStatus() == FIRST_LOAD) {
                int total = firstLoad(response); // 总条数
                ...
                onLoad();
                int total = loadMore(response); // 总条数
                int pageNum = total / CURR_MAX_PAGE_SIZE; // 第几页
                int pageSize = total % CURR_MAX_PAGE_SIZE; // 多少条
                if (pageSize != 0) pageNum++;
                if (pageNum <= currPage) { // 不能加载更多
                    setLoadMore(false);
//                    SDToast.showShort("没有数据了");
                    mListView.setPullLoadEnable(false);
                } else {
                ...
                onLoad();
                int total = refresh(response); // 总条数
                int pageNum = total / CURR_MAX_PAGE_SIZE; // 第几页
                int pageSize = total % CURR_MAX_PAGE_SIZE; // 多少条
                if (pageSize != 0) pageNum++;
                if (pageNum == 0) {
         ...
         * @return 返回当前条数
         */
        protected abstract int firstLoad(JSONObject response);
         ...
         * @return 返回值 当前总记录数
         */
        protected abstract int loadMore(JSONObject response);
         ...
         * @return 返回当前条数
         */
        protected abstract int refresh(JSONObject response);
    }
