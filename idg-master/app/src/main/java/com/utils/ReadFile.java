...
import tablayout.view.dialog.DownloadDialog;
...
public class ReadFile
{
     ...
     * @param entity
     */
    public void openFileAnnexdata(Annexdata entity)
    {
        ...
        try
        {
            Annexdata data = db.findFirst(Selector.from(Annexdata.class).where(WhereBuilder.b("path", "=", entity.getPath())));
            if (data != null)
            { // 打开文件
    ...
    private void startOss(final File file, final Annexdata entity)
    {
        ...
        download = FileDownloadUtil.resumableDownload(context.getApplication(), entity.getPath(), entity.getPath(), entity.getAnnexWay(), "", new FileDownload.DownloadListener()
        {
            @Override
