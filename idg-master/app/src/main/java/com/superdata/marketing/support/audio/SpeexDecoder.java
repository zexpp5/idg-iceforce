...
//    private MyHandler mHandler = null;
    ...
    public SpeexDecoder(File srcPath, Handler mHandler) throws Exception
    {
        this.srcPath = srcPath;
    ...
    {
        SDLogUtil.debug("sampleRate============" + sampleRate);
        ...
        try
        {
            ...
            while (true)
            {
                ...
                for (curseg = 0; curseg < segments; curseg++)
                {
                    ...
                    if (Thread.interrupted())
                    {
                        SDLogUtil.debug(srcPath + "be interrupted !!!  for");
                        dis.close();
                        track.stop();
                        return;
                    }
    ...
    public Speex getSpeexDecoder()
    {
        return speexDecoder;
    }

    public void setSpeexDecoder(Speex speexDecoder)
    {
    ...
    public int getCurrentPostion()
    {
        ...
        return track.getPlaybackHeadPosition() / track.getSampleRate();
    }
    ...
    public int getTotalTime()
    {
        ...
        if (track.getChannelConfiguration() == AudioFormat.CHANNEL_IN_MONO
                || track.getChannelConfiguration() == AudioFormat.CHANNEL_OUT_MONO)
        {
            return (int) ((srcPath.length() * 8) / track.getSampleRate());
        } else
        {
            return (int) ((srcPath.length() * 8) / track.getSampleRate() / 2);
        }
    }
