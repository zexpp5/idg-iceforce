...
public class SpeexWriter implements Runnable {
    ...
    public void run() {
        SDLogUtil.debug("write thread runing");
        while (this.isRecording() || list.size() > 0) {

            if (list.size() > 0) {
                pData = list.remove(0);
                // gauss_packageSize/2
                SDLogUtil.debug("pData size=" + pData.size);

                client.writeTag(pData.processed, pData.size);

                SDLogUtil.debug("list size = {}" + list.size());
            } else {
                try {
                    Thread.sleep(20);
        ...
        SDLogUtil.debug("write thread exit");
        stop();
    }

    public void putData(final byte[] buf, int size) {

        SDLogUtil.debug("after convert. size=====================[640]:" + size);
