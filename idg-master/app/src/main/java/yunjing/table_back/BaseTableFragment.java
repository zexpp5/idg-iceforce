...
import yunjing.table.NoScrollBottomHorizontalScrollView;
...
public abstract class BaseTableFragment extends Fragment {
    ...
    public View tv_name_below;//Tab下面那条线
    ...
    public RelativeLayout rl_table;//root


    private NoScrollHorizontalScrollView sv_normalgoods_title;//不可滑动的顶部左侧的ScrollView
    public NoScrollBottomHorizontalScrollView sv_bottom_title;//底部的合计
    ...
    public boolean isTwoLineBottom = false; //底部

    public View warehouse_tatal_ll;//仓库的底部合计
    ...
    public View test1;//有合计的时候右边详情向上滑动错位
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        view = inflater.inflate(R.layout.erp_table_lst_view_, container, false);
        ...
        warehouse_tatal_ll = view.findViewById(R.id.warehouse_tatal_ll);
        /**
         * 设置footview高度
         */
        nameFootView.setMinimumHeight(120);
        infoFootView.setMinimumHeight(120);
        ...
        if (!isBottom) {
            lv_normalgoodname.addFooterView(new View(getActivity()));
            lv_normalgood_info.addFooterView(new View(getActivity()));
        }
        if (isBottom) {
...
//        }
        ...
        if (isTwoLineBottom) {//底部两行合计
            warehouse_tatal_ll.setVisibility(View.VISIBLE);
            //动态改变位置
            setBottomLocation();
        }
    ...
    protected void setBottomLocation() {
        ...
        layoutParams1.addRule(RelativeLayout.ABOVE, R.id.warehouse_tatal_ll);
        layoutParams2.addRule(RelativeLayout.ABOVE, R.id.warehouse_tatal_ll);
     ...
     * @param position
     */
    public void setListViewBackGround(ViewHolder viewHolder, int position) {
        if ((position + 1) % 2 == 0) {
...
//        test1 = view.findViewById(R.id.test1);
        tv_name_below = view.findViewById(R.id.tv_name_below);
...
//        tv_name.setTypeface(BaseApplication.createWRYHTypeFace());
        ...
        sv_bottom_title = (NoScrollBottomHorizontalScrollView) view.findViewById(R.id.sv_bottom_total);//底部
...
//        }
    ...
    private void combination(final ListView lvName, final ListView lvDetail, final NoScrollHorizontalScrollView title, final LinkedHorizontalScrollView content, final NoScrollBottomHorizontalScrollView bottom) {
        /**
         * 顶部title
         */
         ...
         */
//        if(isBottom){
//            bottom.setMyScrollChangeListener(new NoScrollHorizontalScrollView.NoScrollHorizontalScrollViewListener() {
//                @Override
//                public void onscroll(NoScrollHorizontalScrollView view, int x, int y, int oldl, int oldt) {
//                    content.scrollTo(x, y);//详情
//                }
...
//        this.testAddContentRows(contentMap);
        ...
        adapter.setmTextListener(new CustomeTableViewAdapter.TextListener() {

            @Override
            public void setTextClick(View view) {

                view1 = view;
                setTextClickAndOrder();
            }
        });
        adapter.notifyDataSetChanged();
    }

    public void setTextClickAndOrder() {

    }

    public void setOrderTextClick() {
    ...
    private void testAddHead(HashMap headMap, String headName) {
        HeadItemCell itemCell = new HeadItemCell(headName, 100);//每一列的宽度
