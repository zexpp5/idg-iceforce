 ...
 * 列表dialog,主要是针对 人事事务 申请xxx（不是通用dialog）
 */
...
public class DialogListView extends Dialog {
    ...
    private DialogListView(Builder builder) {
    ...
    private DialogListView(Builder builder, int resStyle) {
    ...
    public void setData(List<ListDialogBean> newData) {
        if (dialogListAdapter != null && null != newData) {
            ...
            if (newData.size() > 5) {
                ...
                layoutParams.height = DisplayUtil.dp2px(mContext, 250);
    ...
    public static final class Builder {
        ...
        public DialogListView build() {
            if (resStyle != -1) {
                return new DialogListView(this, resStyle);
            } else {
                return new DialogListView(this);
            }
        }
    }
