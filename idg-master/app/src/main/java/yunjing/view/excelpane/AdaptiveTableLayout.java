...
public class AdaptiveTableLayout extends ViewGroup implements ScrollHelper.ScrollHelperListener, AdaptiveTableDataSetObserver {
    ...
    private static final int SHADOW_HEADERS_THICK = 0;
    ...
    /**
     * Container with layout_city state
     */
    ...
    /**
     * Keep layout_city settings
     */
    private AdaptiveTableLayoutSettings mSettings;

    /**
     * Detect all gestures on layout_city.
     */
    ...
    private ScrollCallBackListener mScrollCallBackListener;
    ...
    public void setScrollCallBackListener(ScrollCallBackListener mScrollCallBackListener){
        this.mScrollCallBackListener =mScrollCallBackListener;
    }

    /**
     * @return true if layout_city direction is RightToLeft
     */
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        if (changed) {
            // calculate layout_city width and height
     ...
     * Create wrapper with links between layout_city rows, columns and data rows, columns.
    ...
    public void setAdapter(@Nullable AdaptiveTableAdapter adapter) {
        ...
        if (mSettings.getLayoutHeight() != 0 && mSettings.getLayoutWidth() != 0) {
            // if layout_city has width and height
            initItems();
        }
    }
    ...
    public void scrollBy(int x, int y) {
        ...
        if (mState.getScrollY() + tempY <= 0) {
            ...
            mState.setScrollY(0);
            if (mScrollCallBackListener !=null) {
                mScrollCallBackListener.toTop(true);
                mScrollCallBackListener.toBottom(false);
            }
            ...
            mState.setScrollY(0);
            if (mScrollCallBackListener !=null) {
                mScrollCallBackListener.toTop(false);
                mScrollCallBackListener.toBottom(false);
            }
            ...
            if (mScrollCallBackListener !=null) {
                mScrollCallBackListener.toBottom(true);
                mScrollCallBackListener.toTop(false);
            }
        } else {
    ...
    private void refreshItemViewHolder(@NonNull ViewHolder holder,
        ...
        // calculate view layout_city positions
    ...
    /**
     * Recycle view holder and remove view from layout_city.
     *
    ...
    /**
     * Create and add view holders with views to the layout_city.
     *
    ...
    private void addViewHolder(int row, int column, int itemType) {
        ...
        if (itemType == ViewHolderType.ITEM) {
            ...
            if (createdNewView) {
                // DO NOT REMOVE THIS!! Fix bug with request layout_city "requestLayout() improperly called"
        ...
        if (viewHolder != null) {
            ...
            if (onItemClickListener != null) {
                if (viewHolder.getItemType() == ViewHolderType.ITEM) {
                    if (mManager.getFullHeight()>=e.getY()) {
