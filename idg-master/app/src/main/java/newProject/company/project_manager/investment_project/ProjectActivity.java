...
import newProject.company.project_manager.investment_project.fragment.IceForceFragment;
import newProject.company.project_manager.investment_project.fragment.MyInvestmentFragment;
import newProject.company.project_manager.investment_project.fragment.MyPartnerFragment;
import newProject.company.project_manager.investment_project.fragment.ProjectLibraryFragment;
...
public class  ProjectActivity extends com.cxgz.activity.cxim.base.BaseActivity implements View.OnClickListener
{
    ...
    private IceForceFragment workFragment;
    ...
    protected void init()
    {
        JPushManager.getInstance(BaseApplication.getInstance()).setAlias(DisplayUtil.getUserInfo(ProjectActivity.this, 5));
        ...
        //推送
        ToolMainUtils.getInstance().setPSUnRead(ProjectActivity.this);
        getEventData();
    }

    private void getEventData(){
        ListHttpHelper.getToDoEventList(this, SPUtils.get(this, USER_ACCOUNT, "").toString(),"TODO", "0", "10", new SDRequestCallBack(ToDoListBean.class) {
            @Override
...
//
        ...
        if (workFragment == null || workCircleFragment == null || eventFragment == null || projectLibraryFragment == null ||
                myInvestmentFragment == null)
        {
            initFragment();
        }
    ...
    //设置工作台的未读数量
    public void setOneUnRead(int isShow)
    {
        if (isShow > 0)
        {
    ...
    /*//项目库
    private void setTwoUnRead(int isShow)
    {
        if (isShow > 0)
        {
    ...
    //文档库
    private void setFourUnRead(int isShow)
    {
        if (isShow > 0)
        {
            //buttonFour.setUnreadCount(isShow);
        } else
        {
            buttonFour.setUnreadCount(-1);
        }
    }
    ...
    public void setProjectLibraryIntentData(Intent data){
        projectLibraryData = data;
    }

    public Intent getProjectLibraryIntentData(){
        return projectLibraryData;
    }
    ...
    public void onClick(View v)
    {
        switch (v.getId())
        {
            ...
            case R.id.home_tab_btn_3:
                ...
                startActivity(new Intent(ProjectActivity.this,ProjectPopupActivity.class));
    ...
    private void initPopupWindow(View view){
        ...
        llNew.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(ProjectActivity.this, PotentialProjectsAddActivity.class);
        ...
        llTrack.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(ProjectActivity.this, TrackProgressAddActivity.class);
        ...
        popupWindow.showAtLocation(view, Gravity.NO_GRAVITY,(location[0] + view.getWidth() / 2) - popupWidth / 2, location[1] - popupHeight - 30);
    ...
    {
        workFragment = (IceForceFragment) getSupportFragmentManager().findFragmentByTag("0");
        if (workFragment == null)
            workFragment = new IceForceFragment();

        /*projectLibFragment = (ProjectLibFragment) getSupportFragmentManager().findFragmentByTag("1");
        if (projectLibFragment == null)
            projectLibFragment = new ProjectLibFragment();*/
        ...
        projectLibraryFragment = (ProjectLibraryFragment) getSupportFragmentManager().findFragmentByTag("3");
        if (projectLibraryFragment == null)
            projectLibraryFragment = new ProjectLibraryFragment();

        myInvestmentFragment = (MyInvestmentFragment) getSupportFragmentManager().findFragmentByTag("4");
        if (myInvestmentFragment == null)
            myInvestmentFragment = new MyInvestmentFragment();

        myPartnerFragment = (MyPartnerFragment) getSupportFragmentManager().findFragmentByTag("4");
        if (myPartnerFragment == null)
            myPartnerFragment = new MyPartnerFragment();

        String flag = (String)SPUtils.get(this, SPUtils.ROLE_FLAG, "0");
        if (flag.equals("205")) {
            fragmentsList = new Fragment[]{workFragment,eventFragment, workCircleFragment, projectLibraryFragment, myInvestmentFragment};
        }else {
            fragmentsList = new Fragment[]{workFragment, eventFragment,workCircleFragment,  projectLibraryFragment, myPartnerFragment};
        }
        FragmentTransaction transaction = manager.beginTransaction();
        ...
        transaction.add(R.id.fragment_container, projectLibraryFragment, "3");

        if (flag.equals("205") ){
            transaction.add(R.id.fragment_container, myInvestmentFragment, "4");
        }else {
            transaction.add(R.id.fragment_container, myPartnerFragment, "4");
        }
        transaction.hide(workFragment);
        transaction.hide(eventFragment);
        transaction.hide(workCircleFragment);
        transaction.hide(projectLibraryFragment);
        if (flag.equals("205") ){
            transaction.hide(myInvestmentFragment);
        }else {
            transaction.hide(myPartnerFragment);
        }
        transaction.commit();
    }
                ...
                if (!ownerBean.getUserId().equals(currentAccount))
                {
                    if (StringUtils.notEmpty(imGroup))
                    {
                        if (StringUtils.notEmpty(imGroup.getGroupName()))
                            MyToast.showToast(ProjectActivity.this, "你已经被" + imGroup.getGroupName() + "移除");
                    } else
                    {
                        MyToast.showToast(ProjectActivity.this, "你已经被移出群聊");
                    }
                }
            } catch (Exception e)
            {
...
//            refresh();
    ...
    {
        groupChangeListener = new ProjectActivity.MyGroupChangeListener();
...
//                    reFreshUnreadCount();
    ...
    private void getGroupList()
    {
        ...
        IMGroupManager.getInstance().getGroupsFromServer(new IMGroupManager.IMGroupListCallBack()
        {
            ...
            {
                MyToast.showToast(ProjectActivity.this, "刷新群组失败");
            }
        });
    }
