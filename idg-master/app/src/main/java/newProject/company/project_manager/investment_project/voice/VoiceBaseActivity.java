...
public abstract class VoiceBaseActivity extends AppCompatActivity {
    ...
    {
        super.onStart();
        BaseApplication.getInstance().addActivity(VoiceBaseActivity.this);
        hideSoftKeyboard();

//        if (!CxSocketManager.getInstance().isConn()) 123
//        {
//            SDLogUtil.debug("IM——重连-没有连接！");
//            CxReconnManager.getInstance(this).triggerReconnEvent();
//        } else
//        {
//            SDLogUtil.debug("IM——重连-有连接！");
//        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        mDbUtils = SDDbHelp.createDbUtils(VoiceBaseActivity.this);
        mDbUtils.configDebug(SDLogUtil.FLAG);

        mHttpHelper = new SDHttpHelper(VoiceBaseActivity.this);
        userDao = new SDUserDao(VoiceBaseActivity.this);
...
// .LayoutParams.MATCH_PARENT));
    ...
    protected void setLeftBack(int resId)
    {
        addLeftBtn(resId, new View.OnClickListener()
        {
            ...
            public void onClick(View view)
            {
                VoiceBaseActivity.super.onBackPressed();
            }
        });
    }
    ...
    private VoiceBaseActivity.PermissionCallback permissionRunnable;
     ...
     */
    public void performCodeWithPermission(@NonNull String permissionDes, VoiceBaseActivity.PermissionCallback runnable, @NonNull String...
            permissions)
    {
...
//                    .show();
            ...
            new AlertDialog.Builder(this)
                    ...
                    .setPositiveButton("允许", new DialogInterface.OnClickListener()
                    {
                        ...
                        {
                            ActivityCompat.requestPermissions(VoiceBaseActivity.this, permissions, requestCode);
                        }
                    }).show();
    ...
    private void showMeetingDoing(final UnReadMessage info)
    {
        DialogTextFilter dialogTextFilter = new DialogTextFilter();
        dialogTextFilter.setContentString("您有一项语音会议 (" + info.map.get("title") + ") 需要您参与,确定前往?");
        dialogTextFilter.setYesColor("#398BEE");
        dialogTextFilter.setNoColor("#398BEE");
        DialogImUtils.getInstance().showCommonDialog(this, dialogTextFilter, new DialogImUtils.OnYesOrNoListener()
        {
            @Override
            public void onYes()
            {
                Intent intentMeeting = new Intent(VoiceBaseActivity.this, MeetingActivity.class);
                Bundle bundle = new Bundle();
                bundle.putString(Constants.EID, info.map.get("meetId") + "");
    ...
    private void showFinish(String meetingString)
    {
        DialogTextFilter dialogTextFilter = new DialogTextFilter();
        dialogTextFilter.setContentString("语音会议(" + meetingString + ")已经结束");
        dialogTextFilter.setYesColor("#398BEE");
        dialogTextFilter.setNoColor("#398BEE");
        dialogTextFilter.setHaveBtn(1);

        DialogImUtils.getInstance().showCommonDialog(VoiceBaseActivity.this, dialogTextFilter, new
