...
import newProject.company.project_manager.investment_project.adapter.TrackProgressMultiAdapter2;
...
public class TrackProgressActivity2 extends BaseActivity {
    @Bind(R.id.title_bar)
    ...
    TrackProgressMultiAdapter2 adapter;
    ...
    protected void init() {
        ...
        mNavigatorBar.setRightImageOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent intent = new Intent(TrackProgressActivity2.this, TrackProgressAddActivity.class);
        ...
        adapter = new TrackProgressMultiAdapter2(followProjectByIdMultiBeen,permission);
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                if (view.getId() == R.id.ll_voice){
                    ...
                    if (followProjectByIdMultiBeen.get(position).getData().getNote().contains(",")) {
                        ...
                        ivVoice.setBackgroundColor(0xffffffff);
                        ivVoice.setImageResource(R.drawable.play_annex_voice);
                        VoiceUtils.getInstance().getVoiceFromNet(TrackProgressActivity2.this, followProjectByIdMultiBeen.get(position).getData().getNote(), ivVoice);
                    }
    ...
    private void initRefresh()
    {
        mRefreshLayout.setOnRefreshListener(new OnRefreshListener()
        {
            ...
            {
                if (DisplayUtil.hasNetwork(TrackProgressActivity2.this))
                {
                    mPage = 0;
    ...
    public void initPopupWindow(View view, final int position) {
        ...
        View contentView = LayoutInflater.from(this).inflate(R.layout.popupwindow_remind, null);
        ...
        tvContent.setText("警告");
        tvDesc.setText("是否确定删除？");
    ...
    private void delData(final int position){
        ListHttpHelper.delTrackProgressDataByNoteId(this, followProjectByIdMultiBeen.get(position).getData().getNoteId(), new SDRequestCallBack(IDGBaseBean.class) {
            @Override
            public void onRequestFailure(HttpException error, String msg) {
            ...
            public void onRequestSuccess(SDResponseInfo responseInfo) {
                ...
                if ("success".equals(bean.getData().getCode())){
                    /*followProjectByIdMultiBeen.remove(followProjectByIdMultiBeen.get(position).getData());
                    adapter.notifyDataSetChanged();*/
                    mPage = 0;
                    mBackListSize = 0;
                    getData();
    ...
    private void initVoiceListPopupWindow(View view,FollowProjectByIdBaseBean bean){
        ...
        View contentView = LayoutInflater.from(TrackProgressActivity2.this).inflate(R.layout.popupwindow_voice_list, null);
        RecyclerView voiceRecyclerView = (RecyclerView)contentView.findViewById(R.id.recycler_view);
        voiceRecyclerView.setLayoutManager(new LinearLayoutManager(TrackProgressActivity2.this));
        ...
        adapter.setOnItemChildClickListener(new BaseQuickAdapter.OnItemChildClickListener() {
            ...
            public void onItemChildClick(BaseQuickAdapter adapter, View view, int position) {
                switch (view.getId()){
                    case R.id.iv_voice:
                        VoiceUtils.getInstance().getVoiceFromNet(TrackProgressActivity2.this,((VoiceListBean)adapter.getData().get(position)).getUrl(),(ImageView) view);
                        break;
                }
            }
        });
