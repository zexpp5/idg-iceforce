...
public class ProjectLibraryFragment extends BaseFragment {
    ...
    public void postSearchData(){
        Intent data = ((ProjectActivity) getActivity()).getProjectLibraryIntentData();
        mPage = 1;
        mBackListSize = 0;
        personInCharge = "";
        projInvestedStatus = "";
    ...
    public void getData() {
        ListHttpHelper.getProjectLibraryList(getActivity(), SPUtils.get(getActivity(), USER_ACCOUNT, "").toString(), "", mPage + "", "10", "", "", personInCharge, comIndu, projInvestedStatus, stsId, projQueryStr, teamQueryStr, queryStr, new SDRequestCallBack(PotentialProjectsPersonalBean.class) {
            @Override
