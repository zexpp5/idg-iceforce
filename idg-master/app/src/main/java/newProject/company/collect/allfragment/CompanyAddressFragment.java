...
import newProject.company.collect.allbean.AddressDetailBean;
 ...
 * 公司地址
 */
public class CompanyAddressFragment extends ERPSumbitBaseFragment {
    ...
    protected int getContentLayout() {
        return R.layout.company_address_fragment;
    }
    ...
    protected void init(View view) {
        ...
        mAddress = (FontEditext) view.findViewById(R.id.company_address_ed);
        mAddress.setFilters(new InputFilter[]{new InputFilter.LengthFilter(30),DisplayUtil.getFilter(getActivity())});
        mPhone = (FontEditext) view.findViewById(R.id.company_phone_ed);
    ...
    public void getFragmentData(long mEid){
        CollectHttpHelper.getAddress(mEid+"", mHttpHelper, new SDRequestCallBack(AddressDetailBean.class) {
            @Override
            ...
            public void onRequestSuccess(SDResponseInfo responseInfo) {
                AddressDetailBean detailBean = (AddressDetailBean) responseInfo.getResult();
                if (detailBean != null) {
                    if (detailBean.getData() != null) {
                        AddressDetailBean.DataBean bean = detailBean.getData();
                        if (bean != null) {
                            String selfId=bean.getCompanyAddress().getCreateId()+"";
    ...
    public void setInfo(AddressDetailBean bean, boolean isEdit){
        if (bean.getData()==null){
            return;
        }
        controllerLayout(isEdit);
        AddressDetailBean.DataBean.CompanyAddressBean comNotice = bean.getData().getCompanyAddress();
        if (comNotice==null){
            return;
        }
        ...
        //内容
        if (comNotice.getAddress()!=null) {
            mAddress.setText(comNotice.getAddress());
        }
        //附件
    ...
    public void commitData(long mEid){
        ...
        if (mEid==0){
            if (StringUtils.notEmpty(files)||StringUtils.notEmpty(voice)|| StringUtils.notEmpty(imgPaths)){
                postFile(name, address,phone,"");
            }else{
                postData(name, address,phone,"","");
            }
        }else {
            postData(name, address, phone,"", mEid + "");
        }
    }
     ...
     * 提交附件后提交
     */
    public void postFile(final String name, final String address, final String phone, final String eid){
        BaseHttpHelper.submitFileApi(getActivity().getApplication(), "", isOriginalImg, files, imgPaths, voice, new FileUpload.UploadListener() {
            ...
            public void onSuccess(SDResponseInfo responseInfo, HashMap<String, Object> result) {
                ...
                //附件上传完成后的。调用这个接口。
                postData(name,address,phone,new Gson().toJson(callBean.getData()),eid);
            }
     ...
     * 通知
     */
    public void postData(String name,String address, String phone,String annex,String eid){

        CollectHttpHelper.commitAddress(name, address,phone,annex,eid,
